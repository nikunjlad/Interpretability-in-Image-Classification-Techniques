...
class SemanticSegmentation:
    ...
    def __init__(self, model, image, show_orig=True, dev="cuda", arch_name="fcn_resnet50", img_name="sample"):
        self.model = model
        self.image = image
        ...
        self.name = lambda x: "FCN" if self.arch_name.split("_")[0] == "fcn" else "DeepLab-V3"
    ...
    def decode_segmap(self, image, orig, nc=21):
        ...
        original = cv2.resize(np.array(self.image),
        ...
        cv2.putText(original, "Original Image", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
        cv2.putText(rgba, "Semantic Segmented using {}".format(str(self.name(self.arch_name))), (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)

        # stack vertically both the images for comparison
        out = np.vstack((original, rgba))
    ...
    def segment(self):
        ...
        out = self.model.to(self.dev)(inp)['out']  # apply model on the input tensor
        om = torch.argmax(out.squeeze(), dim=0).detach().cpu().numpy()  # acquire output tensor and convert to numpy

        # create segmentation map
        rgb = self.decode_segmap(om, orig=self.image)

        # write segmentation map out to disk
        print("Segmentation complete! Saving {} segmented maps!".format(str(self.name(self.arch_name))))
        cv2.imwrite("results/" + str(self.name(self.arch_name)) + "_" + self.img_name.split(".")[0] + "_seg_map.png",
                    rgb)
...
def semantic(image_path, arch, cuda):
    ...
    s = SemanticSegmentation(model, image=img, show_orig=False, dev=device, arch_name=arch, img_name=img_name)
    print("Segmenting image using {} network!".format(str(s.name(arch))))
    s.segment()
    toc = time.time()
    print("Inference time for {} network is {} secs!".format(str(s.name(arch)), str(toc - tic)))
    sys.exit(1)


@main.command
@click.option("-i", "--image_path", type=str, required=True)
@click.option("-a", "--arch1", type=click.Choice(model_names), required=True)
@click.option("-a", "--arch2", type=click.Choice(model_names), required=True)
